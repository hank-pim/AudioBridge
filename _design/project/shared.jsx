...
function TopBar({ active="streams", alerts=2 }) {
  ...
  return (
    <header className="ab-topbar">
      ...
      <nav>
        {["streams", "routing", "diagnostics", "events", "settings"].map(k => (
          <a key={k} data-active={k === active ? "" : null}>{k}</a>
        ))}
      </nav>
      <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <button className="ab-btn" data-variant="ghost" style={{ padding: "0 8px" }}><Icon.cog /></button>
      </div>
    </header>
  );
}
